...
public class NfeSvrsService extends NfeAnService {
    ...
    public <T extends SefazRequest<TConsCad, TRetConsCad>> Pair<TConsCad, TRetConsCad> queryRegister(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.getConfig().production()) {
            ...
            br.inf.portalfiscal.nfe.wsdl.query_register.svrs.hom.CadConsultaCadastro4Soap ws = ((br.inf.portalfiscal.nfe.wsdl.query_register.svrs.hom.CadConsultaCadastro4) getSoapService().homQueryRegister()).getCadConsultaCadastro4Soap12();
            data.getConfigureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.getConfig()).build());

            br.inf.portalfiscal.nfe.wsdl.query_register.svrs.hom.NfeDadosMsg msg = new br.inf.portalfiscal.nfe.wsdl.query_register.svrs.hom.NfeDadosMsg();
            msg.getContent().add(envio);

            br.inf.portalfiscal.nfe.wsdl.query_register.svrs.hom.NfeResultMsg resultMsg = ws.consultaCadastro(msg);
