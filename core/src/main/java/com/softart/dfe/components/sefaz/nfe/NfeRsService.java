...
public final class NfeRsService extends NfeAnService {
    ...
    public <T extends SefazRequest<TConsCad, TRetConsCad>> Pair<TConsCad, TRetConsCad> queryRegister(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        TRetConsCad retorno;
        if (data.config().production()) {
            br.inf.portalfiscal.nfe.wsdl.query_register.rs.prod.CadConsultaCadastro4Soap12 ws = ((br.inf.portalfiscal.nfe.wsdl.query_register.rs.prod.CadConsultaCadastro4) getSoapService().prodQueryRegister()).getCadConsultaCadastro4Soap12();
            data.configureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.config()).build());

            br.inf.portalfiscal.nfe.wsdl.query_register.rs.prod.NfeDadosMsg msg = new br.inf.portalfiscal.nfe.wsdl.query_register.rs.prod.NfeDadosMsg();
            msg.getContent().add(envio);

            br.inf.portalfiscal.nfe.wsdl.query_register.rs.prod.NfeResultMsg resultMsg = ws.consultaCadastro(msg);
