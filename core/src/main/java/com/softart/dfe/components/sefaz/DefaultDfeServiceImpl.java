...
final class DefaultDfeServiceImpl extends DfeServiceFactory {
    ...
    public NfceService getNfceService(NfceConfig config) throws NoProviderFound, SoapServiceGeneralException {
        return ReflectionUtils.newInstance(getNfceServices().stream().filter(it -> it.allow(config.webServiceUF(), config.environment(), config.emission())).findFirst().orElseThrow(NoProviderFound::new).getClass()).withSoapService(SoapService.getInstance().getNfceSoapService(config));
    }

    @Override
    public NfeService getNfeService(NfeConfig config) throws NoProviderFound, SoapServiceGeneralException {
        return ReflectionUtils.newInstance(getNfeServices().stream().filter(it -> it.allow(config.webServiceUF(), config.environment(), config.emission())).findFirst().orElseThrow(NoProviderFound::new).getClass()).withSoapService(SoapService.getInstance().getNfeSoapService(config));
    }

    @Override
    public CteService getCteService(CteConfig config) throws NoProviderFound, SoapServiceGeneralException {
        return ReflectionUtils.newInstance(getCteServices().stream().filter(it -> it.allow(config.webServiceUF(), config.environment(), config.emission())).findFirst().orElseThrow(NoProviderFound::new).getClass()).withSoapService(SoapService.getInstance().getCteSoapService(config));
    }

    @Override
    public MdfeService getMdfeService(MdfeConfig config) throws NoProviderFound, SoapServiceGeneralException {
        return ReflectionUtils.newInstance(getMdfeServices().stream().filter(it -> it.allow(config.webServiceUF(), config.environment(), config.emission())).findFirst().orElseThrow(NoProviderFound::new).getClass()).withSoapService(SoapService.getInstance().getMdfeSoapService(config));
    }
}
