    ...
    public void storeSendCancel(Store<br.inf.portalfiscal.nfe.event_cancel.TEnvEvento> o) throws StorageException {
        try {
            if (Objects.nonNull(o.getData()) && !o.getData().getEvento().isEmpty() && Objects.nonNull(o.getXml())) {
                getStorageService().writeSend(o, NFStorageKey.NF_EVENT, o.getData().getEvento().get(0).getInfEvento().getChNFe() + "-" + o.getData().getEvento().get(0).getInfEvento().getTpEvento());
            }
        } catch (Exception e) {
            throw new StorageException(e);
        }
    ...
    public void storeEnvNfe(Store<TEnviNFe> o) throws StorageException {
        try {
            if (Objects.nonNull(o.getData()) && Objects.nonNull(o.getXml())) {
                getStorageService().writeSend(o.getConfig(), NFStorageKey.NF_AUTHORIZATION, xmlNameWithTime(AccessKeyParserFactory.nfe().fromId(o.getData().getNFe().get(0).getInfNFe().getId())), o.getXml());
            }
        } catch (Exception e) {
            throw new StorageException(e);
        }
