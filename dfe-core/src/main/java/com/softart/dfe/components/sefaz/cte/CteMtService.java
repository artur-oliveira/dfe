...
public final class CteMtService extends CteAnService {
    ...
    public <T extends SefazRequest<TEvento, TRetEvento>> Pair<TEvento, TRetEvento> event(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.config().production()) {
            ...
            data.configureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.config()).overridePortAddress("https://cte.sefaz.mt.gov.br/ctews2/services/CteRecepcaoEvento").build());
            ...
            data.configureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.config()).overridePortAddress("https://homologacao.sefaz.mt.gov.br/ctews2/services/CteRecepcaoEvento").build());
    ...
    public <T extends SefazRequest<TConsSitCTe, TRetConsSitCTe>> Pair<TConsSitCTe, TRetConsSitCTe> querySituation(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.config().production()) {
            ...
            data.configureProvider().configure(ProviderConfig.builder().overridePortAddress("https://cte.sefaz.mt.gov.br/ctews/services/CteConsulta").port((BindingProvider) ws).config(data.config()).build());
    ...
    public <T extends SefazRequest<TEnviCTe, TRetEnviCTe>> Pair<TEnviCTe, TRetEnviCTe> reception(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.config().production()) {
            ...
            data.configureProvider().configure(ProviderConfig.builder().overridePortAddress("https://cte.sefaz.mt.gov.br/ctews/services/CteRecepcao").port((BindingProvider) ws).config(data.config()).build());
    ...
    public <T extends SefazRequest<TConsReciCTe, TRetConsReciCTe>> Pair<TConsReciCTe, TRetConsReciCTe> queryReceipt(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.config().production()) {
            br.inf.portalfiscal.cte.wsdl.return_reception.mt.prod.CteRetRecepcaoSoap12 ws = ((br.inf.portalfiscal.cte.wsdl.return_reception.mt.prod.CteRetRecepcao) getSoapService().prodQueryReceipt()).getCteRetRecepcaoSoap12();

            data.configureProvider().configure(ProviderConfig.builder().overridePortAddress("https://cte.sefaz.mt.gov.br/ctews/services/CteRetRecepcao").port((BindingProvider) ws).config(data.config()).build());

            br.inf.portalfiscal.cte.wsdl.return_reception.mt.prod.CteDadosMsg msg = new br.inf.portalfiscal.cte.wsdl.return_reception.mt.prod.ObjectFactory().createCteDadosMsg();
            msg.getContent().add(envio);


            br.inf.portalfiscal.cte.wsdl.return_reception.mt.prod.CteCabecMsg cabec = new br.inf.portalfiscal.cte.wsdl.return_reception.mt.prod.ObjectFactory().createCteCabecMsg();
            cabec.setCUF(data.config().webServiceUF().getCode());
            cabec.setVersaoDados(envio.getValue().getVersao());

            br.inf.portalfiscal.cte.wsdl.return_reception.mt.prod.CteRetRecepcaoResult resultMsg = ws.cteRetRecepcao(msg);

            if (!resultMsg.getContent().isEmpty())
                retorno = (TRetConsReciCTe) ((JAXBElement<?>) resultMsg.getContent().get(0)).getValue();
        } else {
            br.inf.portalfiscal.cte.wsdl.return_reception.mt.hom.CteRetRecepcaoSoap12 ws = ((br.inf.portalfiscal.cte.wsdl.return_reception.mt.hom.CteRetRecepcao) getSoapService().homQueryReceipt()).getCteRetRecepcaoSoap12();
            data.configureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.config()).build());

            br.inf.portalfiscal.cte.wsdl.return_reception.mt.hom.CteDadosMsg msg = new br.inf.portalfiscal.cte.wsdl.return_reception.mt.hom.ObjectFactory().createCteDadosMsg();
            msg.getContent().add(envio);

            br.inf.portalfiscal.cte.wsdl.return_reception.mt.hom.CteCabecMsg cabec = new br.inf.portalfiscal.cte.wsdl.return_reception.mt.hom.ObjectFactory().createCteCabecMsg();
            cabec.setCUF(data.config().webServiceUF().getCode());
            cabec.setVersaoDados(envio.getValue().getVersao());

            br.inf.portalfiscal.cte.wsdl.return_reception.mt.hom.CteRetRecepcaoResult resultMsg = ws.cteRetRecepcao(msg);
    ...
    public <T extends SefazRequest<TConsStatServ, TRetConsStatServ>> Pair<TConsStatServ, TRetConsStatServ> statusService(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.config().production()) {
            ...
            data.configureProvider().configure(ProviderConfig.builder().overridePortAddress("https://cte.sefaz.mt.gov.br/ctews/services/CteStatusServico").port((BindingProvider) ws).config(data.config()).build());
