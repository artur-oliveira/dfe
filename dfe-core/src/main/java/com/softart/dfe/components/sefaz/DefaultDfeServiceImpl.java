...
final class DefaultDfeServiceImpl extends DfeServiceFactory {
    ...
    private final Collection<Cte4Service> cte4Services = ReflectionUtils.findAllClasses(PackageFinder.builder().packages(Collections.singleton("com.softart.dfe.components.sefaz.cte4")).assignables(Collections.singleton(Cte4Service.class)).excludeClasses(Collections.singleton(com.softart.dfe.components.sefaz.cte4.CteAnService.class)).build()).stream().map(it -> (Cte4Service) ReflectionUtils.newInstance(it)).toList();
