...
//            br.inf.portalfiscal.nfe.wsdl.authorization.go.hom.NfeResultMsg resultMsg = ws.nfeAutorizacaoLote(msg);
    ...
    public <T extends SefazRequest<br.inf.portalfiscal.nfe.event_generic.TEnvEvento, br.inf.portalfiscal.nfe.event_generic.TRetEnvEvento>> Pair<br.inf.portalfiscal.nfe.event_generic.TEnvEvento, br.inf.portalfiscal.nfe.event_generic.TRetEnvEvento> event(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.config().production()) {
            br.inf.portalfiscal.nfe.wsdl.event_generic.go.prod.NFeRecepcaoEvento4Service ws = ((br.inf.portalfiscal.nfe.wsdl.event_generic.go.prod.NFeRecepcaoEvento4) getSoapService().prodEvent()).getNFeRecepcaoEvento4ServicePort();

            data.configureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.config()).build());

            br.inf.portalfiscal.nfe.wsdl.event_generic.go.prod.NfeDadosMsg msg = new br.inf.portalfiscal.nfe.wsdl.event_generic.go.prod.ObjectFactory().createNfeDadosMsg();
            msg.getContent().add(envio);
            br.inf.portalfiscal.nfe.wsdl.event_generic.go.prod.NfeResultMsg resultMsg = ws.nfeRecepcaoEvento(msg);

            if (!resultMsg.getContent().isEmpty())
                retorno = ((JAXBElement<br.inf.portalfiscal.nfe.event_generic.TRetEnvEvento>) resultMsg.getContent().getFirst()).getValue();
        } else {
            br.inf.portalfiscal.nfe.wsdl.event_generic.go.hom.NFeRecepcaoEvento4Service ws = ((br.inf.portalfiscal.nfe.wsdl.event_generic.go.hom.NFeRecepcaoEvento4) getSoapService().homEvent()).getNFeRecepcaoEvento4ServicePort();
            data.configureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.config()).build());

            br.inf.portalfiscal.nfe.wsdl.event_generic.go.hom.NfeDadosMsg msg = new br.inf.portalfiscal.nfe.wsdl.event_generic.go.hom.ObjectFactory().createNfeDadosMsg();
            msg.getContent().add(envio);

            br.inf.portalfiscal.nfe.wsdl.event_generic.go.hom.NfeResultMsg resultMsg = ws.nfeRecepcaoEvento(msg);
    ...
    public <T extends SefazRequest<TConsCad, TRetConsCad>> Pair<TConsCad, TRetConsCad> queryRegister(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.config().production()) {
            ...
            retorno = ((JAXBElement<TRetConsCad>) resultMsg.getContent().getFirst()).getValue();
        } else {
