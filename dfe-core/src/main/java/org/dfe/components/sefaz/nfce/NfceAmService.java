...
public final class NfceAmService implements NfceService {
    ...
    public <T extends SefazRequest<br.inf.portalfiscal.nfe.event_generic.TEnvEvento, br.inf.portalfiscal.nfe.event_generic.TRetEnvEvento>> Pair<br.inf.portalfiscal.nfe.event_generic.TEnvEvento, br.inf.portalfiscal.nfe.event_generic.TRetEnvEvento> event(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.config().production()) {
            br.inf.portalfiscal.nfce.wsdl.event_generic.am.prod.RecepcaoEvento4Soap ws = ((br.inf.portalfiscal.nfce.wsdl.event_generic.am.prod.RecepcaoEvento4) getSoapService().prodEvent()).getRecepcaoEvento4Soap12();

            data.configureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.config()).build());

            br.inf.portalfiscal.nfce.wsdl.event_generic.am.prod.NfeDadosMsg msg = new br.inf.portalfiscal.nfce.wsdl.event_generic.am.prod.ObjectFactory().createNfeDadosMsg();
            msg.getContent().add(envio);
            br.inf.portalfiscal.nfce.wsdl.event_generic.am.prod.NfeResultMsg resultMsg = ws.nfeRecepcaoEvento(msg);

            if (!resultMsg.getContent().isEmpty())
                retorno = ((JAXBElement<br.inf.portalfiscal.nfe.event_generic.TRetEnvEvento>) resultMsg.getContent().getFirst()).getValue();
        } else {
            br.inf.portalfiscal.nfce.wsdl.event_generic.am.hom.RecepcaoEvento4Soap ws = ((br.inf.portalfiscal.nfce.wsdl.event_generic.am.hom.RecepcaoEvento4) getSoapService().homEvent()).getRecepcaoEvento4Soap12();
            data.configureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.config()).build());

            br.inf.portalfiscal.nfce.wsdl.event_generic.am.hom.NfeDadosMsg msg = new br.inf.portalfiscal.nfce.wsdl.event_generic.am.hom.ObjectFactory().createNfeDadosMsg();
            msg.getContent().add(envio);

            br.inf.portalfiscal.nfce.wsdl.event_generic.am.hom.NfeResultMsg resultMsg = ws.nfeRecepcaoEvento(msg);
