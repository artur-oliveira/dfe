...
public final class NfcePrService implements NfceService {
    ...
    public <T extends SefazRequest<br.inf.portalfiscal.nfe.event_generic.TEnvEvento, br.inf.portalfiscal.nfe.event_generic.TRetEnvEvento>> Pair<br.inf.portalfiscal.nfe.event_generic.TEnvEvento, br.inf.portalfiscal.nfe.event_generic.TRetEnvEvento> event(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.config().production()) {
            br.inf.portalfiscal.nfce.wsdl.event_generic.pr.prod.NFeRecepcaoEvento4Soap12 ws = ((br.inf.portalfiscal.nfce.wsdl.event_generic.pr.prod.NFeRecepcaoEvento4) getSoapService().prodEvent()).getNFeRecepcaoEvento4ServicePort();

            data.configureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.config()).build());

            br.inf.portalfiscal.nfce.wsdl.event_generic.pr.prod.NfeDadosMsg msg = new br.inf.portalfiscal.nfce.wsdl.event_generic.pr.prod.ObjectFactory().createNfeDadosMsg();
            msg.getContent().add(envio);
            br.inf.portalfiscal.nfce.wsdl.event_generic.pr.prod.NfeResultMsg resultMsg = ws.nfeRecepcaoEventoNF(msg);

            if (!resultMsg.getContent().isEmpty())
                retorno = ((JAXBElement<br.inf.portalfiscal.nfe.event_generic.TRetEnvEvento>) resultMsg.getContent().getFirst()).getValue();
        } else {
            br.inf.portalfiscal.nfce.wsdl.event_generic.pr.hom.NFeRecepcaoEvento4Soap12 ws = ((br.inf.portalfiscal.nfce.wsdl.event_generic.pr.hom.NFeRecepcaoEvento4) getSoapService().homEvent()).getNFeRecepcaoEvento4ServicePort();
            data.configureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.config()).build());

            br.inf.portalfiscal.nfce.wsdl.event_generic.pr.hom.NfeDadosMsg msg = new br.inf.portalfiscal.nfce.wsdl.event_generic.pr.hom.ObjectFactory().createNfeDadosMsg();
            msg.getContent().add(envio);

            br.inf.portalfiscal.nfce.wsdl.event_generic.pr.hom.NfeResultMsg resultMsg = ws.nfeRecepcaoEventoNF(msg);
