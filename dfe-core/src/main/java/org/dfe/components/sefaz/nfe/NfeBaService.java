...
public final class NfeBaService extends NfeAnService {
    ...
    public <T extends SefazRequest<br.inf.portalfiscal.nfe.event_generic.TEnvEvento, br.inf.portalfiscal.nfe.event_generic.TRetEnvEvento>> Pair<br.inf.portalfiscal.nfe.event_generic.TEnvEvento, br.inf.portalfiscal.nfe.event_generic.TRetEnvEvento> event(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.config().production()) {
            br.inf.portalfiscal.nfe.wsdl.event_generic.ba.prod.NFeRecepcaoEvento4Soap ws = ((br.inf.portalfiscal.nfe.wsdl.event_generic.ba.prod.NFeRecepcaoEvento4) getSoapService().prodEvent()).getNFeRecepcaoEvento4Soap();

            data.configureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.config()).build());

            br.inf.portalfiscal.nfe.wsdl.event_generic.ba.prod.NfeDadosMsg msg = new br.inf.portalfiscal.nfe.wsdl.event_generic.ba.prod.ObjectFactory().createNfeDadosMsg();
            msg.getContent().add(envio);
            br.inf.portalfiscal.nfe.wsdl.event_generic.ba.prod.NfeResultMsg resultMsg = ws.nFeRecepcaoEvento(msg);

            if (!resultMsg.getContent().isEmpty())
                retorno = ((JAXBElement<br.inf.portalfiscal.nfe.event_generic.TRetEnvEvento>) resultMsg.getContent().getFirst()).getValue();
        } else {
            br.inf.portalfiscal.nfe.wsdl.event_generic.ba.hom.NFeRecepcaoEvento4Soap ws = ((br.inf.portalfiscal.nfe.wsdl.event_generic.ba.hom.NFeRecepcaoEvento4) getSoapService().homEvent()).getNFeRecepcaoEvento4Soap();
            data.configureProvider().configure(ProviderConfig.builder().port((BindingProvider) ws).config(data.config()).build());

            br.inf.portalfiscal.nfe.wsdl.event_generic.ba.hom.NfeDadosMsg msg = new br.inf.portalfiscal.nfe.wsdl.event_generic.ba.hom.ObjectFactory().createNfeDadosMsg();
            msg.getContent().add(envio);

            br.inf.portalfiscal.nfe.wsdl.event_generic.ba.hom.NfeResultMsg resultMsg = ws.nFeRecepcaoEvento(msg);
    ...
    public <T extends SefazRequest<TInutNFe, TRetInutNFe>> Pair<TInutNFe, TRetInutNFe> inutilization(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.config().production()) {
            ...
            msg.getContent().add(envio);
            br.inf.portalfiscal.nfe.wsdl.inutilization.ba.prod.NfeResultMsg resultMsg = ws.nFeInutilizacao(msg);
            ...
            br.inf.portalfiscal.nfe.wsdl.inutilization.ba.hom.NfeResultMsg resultMsg = ws.nFeInutilizacao(msg);
