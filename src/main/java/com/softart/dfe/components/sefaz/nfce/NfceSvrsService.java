...
public final class NfceSvrsService implements NfceService {
    ...
    public <T extends SefazRequest<TConsStatServ, TRetConsStatServ>> Pair<TConsStatServ, TRetConsStatServ> queryStatusService(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.getConfig().production()) {
            ...
            br.inf.portalfiscal.nfe.wsdl.status_service.svrs.nfce.hom.NFeStatusServico4Soap ws = ((br.inf.portalfiscal.nfe.wsdl.status_service.svrs.nfce.hom.NFeStatusServico4) getSoapService().homReturnAuthorization()).getNFeStatusServico4Soap();
            data.getConfigureProvider().configure((BindingProvider) ws, data.getConfig());
