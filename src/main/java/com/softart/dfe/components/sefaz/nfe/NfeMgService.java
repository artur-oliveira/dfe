...
public final class NfeMgService extends NfeAnService {
    ...
    public <T extends SefazRequest<br.inf.portalfiscal.nfe.event_interested_actor.TEnvEvento, br.inf.portalfiscal.nfe.event_interested_actor.TRetEnvEvento>> Pair<br.inf.portalfiscal.nfe.event_interested_actor.TEnvEvento, br.inf.portalfiscal.nfe.event_interested_actor.TRetEnvEvento> interestedActor(T data) throws SecurityException, ValidationException, ProcessException {
        ...
        if (data.getConfig().production()) {
            br.inf.portalfiscal.nfe.wsdl.event_interested_actor.mg.prod.NFeRecepcaoEvento4Soap ws = ((br.inf.portalfiscal.nfe.wsdl.event_interested_actor.mg.prod.NFeRecepcaoEvento4) getSoapService().prodInterestedActor()).getNFeRecepcaoEvento4Soap12();

            data.getConfigureProvider().configure((BindingProvider) ws, data.getConfig());

            br.inf.portalfiscal.nfe.wsdl.event_interested_actor.mg.prod.NfeDadosMsg msg = new br.inf.portalfiscal.nfe.wsdl.event_interested_actor.mg.prod.ObjectFactory().createNfeDadosMsg();
            msg.getContent().add(envio);
            br.inf.portalfiscal.nfe.wsdl.event_interested_actor.mg.prod.NFeRecepcaoEvento4Result resultMsg = ws.nfeRecepcaoEvento(msg);

            if (!resultMsg.getContent().isEmpty())
                retorno = ((JAXBElement<br.inf.portalfiscal.nfe.event_interested_actor.TRetEnvEvento>) resultMsg.getContent().get(0)).getValue();
        } else {
            br.inf.portalfiscal.nfe.wsdl.event_interested_actor.mg.hom.NFeRecepcaoEvento4Soap ws = ((br.inf.portalfiscal.nfe.wsdl.event_interested_actor.mg.hom.NFeRecepcaoEvento4) getSoapService().homInterestedActor()).getNFeRecepcaoEvento4Soap12();
            data.getConfigureProvider().configure((BindingProvider) ws, data.getConfig());

            br.inf.portalfiscal.nfe.wsdl.event_interested_actor.mg.hom.NfeDadosMsg msg = new br.inf.portalfiscal.nfe.wsdl.event_interested_actor.mg.hom.ObjectFactory().createNfeDadosMsg();
            msg.getContent().add(envio);

            br.inf.portalfiscal.nfe.wsdl.event_interested_actor.mg.hom.NFeRecepcaoEvento4Result resultMsg = ws.nfeRecepcaoEvento(msg);
