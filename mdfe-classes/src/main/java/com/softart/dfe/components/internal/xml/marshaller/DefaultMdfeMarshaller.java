...
final class DefaultMdfeMarshaller extends MdfeMarshallerFactory {
    ...
    public String returnDistribution(RetDistDFeInt data) {
        try {
            try (StringWriter sw = new StringWriter()) {
                MdfeContextFactory.getInstance().getMdfeContext().createMarshaller().marshal(fc.createRetDistDFeInt(data), sw);
                return XMLStringUtils.cleanNamespace(sw.toString());
            }
    ...
    public String sendDistribution(DistDFeInt data) {
        try {
            try (StringWriter sw = new StringWriter()) {
                MdfeContextFactory.getInstance().getMdfeContext().createMarshaller().marshal(fc.createDistDFeInt(data), sw);
                return XMLStringUtils.cleanNamespace(sw.toString());
            }
        } catch (JAXBException | IOException e) {
            throw new MarshallException(e);
